...
{
    [MAPSEC_FAREWARD] = {MAP_GROUP(FAREWARD), MAP_NUM(FAREWARD), HEAL_LOCATION_FAREWARD},
};
...
static u8 GetMapsecType(u16 mapSecId)
{
    ...
    case MAPSEC_FAREWARD:
        return FlagGet(FLAG_VISITED_FAREWARD) ? MAPSECTYPE_CITY_CANFLY : MAPSECTYPE_CITY_CANTFLY;
    case MAPSEC_REMUS:
...
static void CreateFlyDestIcons(void)
{
    ...
    canFlyFlag = FLAG_VISITED_FAREWARD;
    for (mapSecId = MAPSEC_FAREWARD; mapSecId <= MAPSEC_EVER_GRANDE_CITY; mapSecId++)
    {
        ...
        if (!UpdatePaletteFade())
        {
            ...
            if (sFlyMap->choseFlyLocation)
            {
                ...
                case MAPSEC_BATTLE_FRONTIER:
                    ...
                    break;
                case MAPSEC_FAREWARD:
                    SetWarpDestinationToHealLocation(gSaveBlock2Ptr->playerGender == MALE ? HEAL_LOCATION_FAREWARD : HEAL_LOCATION_FAREWARD);
                    break;
                case MAPSEC_EVER_GRANDE_CITY:
